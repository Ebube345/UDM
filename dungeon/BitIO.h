...
#include <sstream>
namespace stl {
	template <class T= std::fstream>
	class m_stream{
	public:T source{};
	public:
	bool put(int32_t& what) { (source.put(what)); return !source.bad();}
	bool get(char& c) { (source.get(c)); return !source.bad();}
	bool is_open() {return true/* source.is_open() */;}
	bool open(std::string const& s, std::ios_base::openmode mode) {source = std::stringstream{s, mode};/* source.open(s, mode) */; return &source;}
	...
	struct BitFile {
		//std::fstream file{};
		m_stream<std::stringstream> file;
		int32_t rack{ 0 };
		int32_t mask{ 0 };
	};
	...
	void outputBits(std::unique_ptr<BitFile>& bitFile, uint32_t code, uint32_t bitCount) {
		int32_t mask;
		mask = 1 << (bitCount - 1);
		while (mask != 0) {
			if (mask & code)
